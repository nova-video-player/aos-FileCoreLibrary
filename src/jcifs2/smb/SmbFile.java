...
import jcifs2.UniAddress;
import jcifs2.netbios.NbtAddress;
import jcifs2.dcerpc.*;
import jcifs2.dcerpc.msrpc.*;
 ...
 */
    ...
    static final int DEFAULT_ATTR_EXPIRATION_PERIOD = 0; /* orig 5000 */
    ...
    public int getType() throws SmbException {
        if( type == 0 ) {
            if( getUncPath0().length() > 1 ) {
                ...
                } else if( tree != null && tree.service.equals( "LPT1:" )) {
                    type = TYPE_PRINTER;
                } else if( tree != null && tree.service.equals( "COMM" )) {
                    type = TYPE_COMM;
                } else {
                    type = TYPE_SHARE;
                }
                ...
                if( addr != null && addr.getAddress() instanceof NbtAddress ) {
...
/**
 * THIS SPECIAL VERSION DOES NOT CHECK ON SERVER BUT IS BASED ONLY ON INFO ALREAYD STORED IN THE INSTANCE
 * Hence it must not be used to query the server.
 * It must be used on an instance we get from a server query like {@link listFiles()}
 * 
 * There seem to be nothing we can do here more than just returning true...
 */
    public boolean canRead_nonetwork() {
        return true; 
    }
 ...
 * will be read-only.
 *
 * @return  <code>true</code> if the resource exists is marked read-only
 */
    public boolean canWrite_nonetwork() {
        return ( attributes & ATTR_READONLY ) == 0;
    }
 ...
 */
    ...
    public boolean isDirectory_noquery() {
        if( getUncPath0().length() == 1 ) {
            return true;
        }
        return ( attributes & ATTR_DIRECTORY ) == ATTR_DIRECTORY;
 ...
 */
     ...
     *         1970 as a <code>long</code> value
     */
        public long lastModified_noquery() {
            if( getUncPath0().length() > 1 ) {
                return lastModified;
            }
            return 0L;
        ...
        while (addr != null) {
            try {
                ...
                try {
                    ...
                    //on nvidia shield as server, doMsrpcShareEnum returns entries with null names,
                    if(entries.length>0&&entries[0].getName()==null) {
