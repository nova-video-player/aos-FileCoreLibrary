...
public abstract class ProxyCancellationChecks {
    ...
    private static final class Proxy extends StreamOverHttp {
        ...
        volatile boolean closeWasInterrupted;
                ...
                @Override public InputStream getInputStream(long from) throws Exception {
                    ...
                    return new InputStream() {
                        ...
                        @Override public void close() throws IOException {
                            ...
                            closeWasInterrupted = Thread.currentThread().isInterrupted();
    ...
    private void assertCooperative(Stage stage, StreamOverHttp.ReadMode mode) throws Exception {
        ...
        try (Socket socket = request(proxy, null)) {
            ...
            assertFalse("Cleanup barrier must retain stalled operations", proxy.awaitIdle(20));
    ...
    private void assertInterruptible(String scheme) throws Exception {
        ...
        try (Socket socket = request(proxy, null)) {
            ...
            assertEquals(1, proxy.closes.get());
            assertFalse("Read cancellation must not poison remote CLOSE", proxy.closeWasInterrupted);
        } finally { proxy.proceed.countDown(); proxy.close(); }
    }

    @Test public void sshjStillInterrupts() throws Exception { assertInterruptible("sshj"); }
    ...
    @Test public void cancellationDoesNotInterruptAnAlreadyRunningRemoteClose() throws Exception {
        Proxy proxy = new Proxy("sshj", Stage.CLOSE);
        try (Socket socket = request(proxy, null)) {
            assertTrue(proxy.entered.await(2, TimeUnit.SECONDS));
            proxy.close();
            assertFalse(proxy.awaitIdle(20));
            assertFalse(proxy.interrupted);
            finish(proxy);
            assertFalse(proxy.interrupted);
            assertFalse(proxy.closeWasInterrupted);
            assertTrue(proxy.awaitIdle(2000));
        } finally { proxy.proceed.countDown(); proxy.close(); }
    }
}
