...
public class StreamOverHttpBufferTest {
    ...
    @Test
    public void jcifsRefillsUseOnlyOneCredit() throws Exception {
        assertFirstReadSize(65536, StreamOverHttp.upstreamBufferSize(new JcifsFileEditor(SMB_URI)));
    }

    @Test
    public void actualBackendSelectionPreservesOtherBufferSizes() throws Exception {
        FileEditor[] editors = {
        ...
        for (FileEditor editor : editors) {
            assertFirstReadSize(81920, StreamOverHttp.upstreamBufferSize(editor));
        }
    }

    @Test
    public void jcifsHttpTransferWorksWithSingleCreditBackend() throws Exception {
        assertSingleCreditTransfer(false);
    }

    @Test
    public void unknownLengthJcifsHttpTransferWorksWithSingleCreditBackend() throws Exception {
        assertSingleCreditTransfer(true);
    }

    private static void assertSingleCreditTransfer(boolean unknownLength) throws Exception {
        RecordingInputStream source = new RecordingInputStream(10 * MIB + 317);
        source.maxRequest = 65536;
        StreamOverHttp proxy = new StreamOverHttp(SMB_URI, "video/mp4") {
        ...
        try {
            ...
            assertEquals(Integer.valueOf(65536), source.requests.get(0));
    ...
    @Test
    public void finalRefillIsLimitedAfterFullBuffer() throws Exception {
        RecordingInputStream source = new RecordingInputStream(3 * MIB);
        InputStream input = StreamOverHttp.bufferResponseInput(source, 65536 + 317, 65536);
        assertArrayEquals(expected(65536 + 317), readAll(input));
        assertEquals(Arrays.asList(65536, 317), source.requests);
        assertEquals(65536 + 317, source.position);
    }
    ...
    private static final class RecordingInputStream extends InputStream {
        ...
        @Override public int read(byte[] bytes, int off, int len) {
            assertTrue("Backend request exceeded its single-credit window: " + len, len <= maxRequest);
            requests.add(len);
